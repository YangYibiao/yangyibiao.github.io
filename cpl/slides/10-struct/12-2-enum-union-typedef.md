...
<div style="flex:1.4;font-size:0.44em;">
...
类型限定符: ==const== (只读) 与 ==volatile== (第 20 章讨论); C99 还有 ==restrict== (只用于指针). 例子:
...
const 与 `#define` 不同: 宏只能给数字/字符/字符串命名, const 可创建==任何类型==的只读对象; const 遵守作用域规则、可在调试器中查看、可对其用 `&` (有地址), 但不能用于常量表达式.
