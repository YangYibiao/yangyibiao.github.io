...
- 程序可能存储在名为hw.c的文件中
- 文件名不重要，但通常需要.c扩展名
- 编译命令：==`gcc hw.c`== 默认存储为`a.out`文件
- 编译命令：==`gcc hw.c -o hw`== 会将可执行程序存为`hw`
...
- ==`#include <stdio.h>`== 引入C语言标准输入/输出库,预处理指令
- ==`main函数`== 主函数，程序入口
...
- [Visual Studio 2022](https://visualstudio.microsoft.com/zh-hans/vs/) 等: 社区版免费、安全检查需配置
...
- int变量：（整数的缩写）存储整数，如`0`，`1`，`392`或`-2553`，整形的取值范围是受限，最大的整数通常是`2 147 483 647`, 16位机最大整数是`32 767`
...
- 关键字（ ==`_Bool`== 、 ==`_Complex`== 和 ==`_Imaginary`== 除外）必须仅使用小写字母编写
