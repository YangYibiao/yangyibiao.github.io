...
int main(void) {
    while (1) {
        int *p = malloc(1000 * sizeof(int));  /* 每次借走 4000 字节 */
        usleep(20000);                        /* 放慢, 便于观察 */
        /* 用完没有 free! */
    }
}
```
...
- 每循环一次就==借走一块内存且永不归还==——可用内存越来越少, 程序最终被系统"杀掉"或整台机器变卡

- 现场观察: 另开一个终端, `top` 里看这个进程的==内存不断上涨==

- 教训: ==malloc 与 free 成对出现==; 用完不再需要的块要及时释放
