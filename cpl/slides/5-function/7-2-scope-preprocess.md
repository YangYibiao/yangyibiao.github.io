...
宏 = ==纯文本替换==。用 `gcc -E` 亲眼看看展开前后:

<div class="fullborder">

| 你写的 | 预处理后 (`gcc -E`) |
| :-- | :-- |
| `#define PI 3.14` | (定义行消失) |
| `area = PI * r * r;` | `area = 3.14 * r * r;` |
| `#define TWO_PI (2*PI)` | |
| `x = TWO_PI * r;` | `x = (2*3.14) * r;` |

</div>

- 现场演示: `gcc -E celsius.c | tail -20`

- 记住"就是文本替换", 后面所有宏的怪现象 (括号、作用域、副作用) 都能解释
...
- <span class="yellow">:fa-weixin:</span> 宏名与左括号之间==不能有空格==: `#define MAX (a,b) ...` 会被当成"无参数的 MAX", 报错信息离现场很远
...
- 临时==屏蔽一大段代码== (段内已有 `/* */` 注释) 时, 用 `#if 0 ... #endif` 比注释更安全——注释不能嵌套, 它可以
...
- ==重构练习==: 第 1 周的猜数字程序, 先写成"全局变量版"(答案变量谁都能改), 再让 AI 帮你改成"参数版"(答案在 main 里算好传给函数), 对比两种写法的优劣
