...
`输入`: 我们有$n$个整数要判断, 输入第一行有一个整数$n$, 接下来有$n$行, 每行有一个待判断的非负整数$x(1 \le n \le 
6000, 0 \le x \le 2 * 10^9)$

`输出`: 输出$n$行, 每行一个整数, 代表每个整数的判断结果. 
...
`输入`: 一行, 一个整数$p$. $(0 \le p \le 10^4)$

`输出`: 一行, 两个整数, 用空格隔开, 分别代表两个结果. 
...
`输出`: 输出若干行，每行代表方程的一组解，每一行的输出形式为$x=x_{i}, y=y_{i}, z=z_{i}$. 题目保证输入所产生的解的个数有穷。
