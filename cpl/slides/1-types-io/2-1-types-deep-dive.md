...
- ==规范化==: 正常数都存成 $1.xxx_2 \times 2^e$ (==xxx 是 23 位 0/1==)——开头的 1 是必然的, ==隐含不存==, 白赚一位精度
...
<!-- slide data-notes="" -->##### 验算一个 float: 0.75
...
| 区间 | 形式 | 指数 e |
| :-- | :-- | :-- |
| [1/2, 1) | $1.xxx_2 \times 2^{-1}$ | -1 |
| (1/4, 1/2), (1/8, 1/4), … | $1.xxx_2 \times 2^{e}$, 一直对半切到 $2^{-126}$ | -2, -3, …, -126 |
| (0, $2^{-126}$) | $0.xxx_2 \times 2^{-126}$ (非规格化数) | 固定 -126 |
| [1, 2) | $1.xxx_2 \times 2^{0}$ | 0 |
| [2, 4) | $1.xxx_2 \times 2^{1}$ | +1 |
| [4, 8) | $1.xxx_2 \times 2^{2}$ | +2 |
</div>
...
指数段==全 0== 与==全 1== 被保留; 浮点数的边界值也都落在它们旁边:
...
| `0` / `-0.0` | 0 | 全 0 | 全 0 |
| 非规格化数 | $0.xxx \times 2^{-126}$ | 全 0 | 非 0 |
| `±∞` | 无穷 | 全 1 | 全 0 |
| `NaN` | 非数 | 全 1 | 非 0 |
</div>

- 最大数不是 $1 \times 2^{127}$ 而是接近 $2 \times 2^{127}$——尾数可以全 1; 最小正正规数不是 $2^{-127}$——指数段 0 被非规格化数征用了

- 两个保留编码落在位模式的两个极端——这是移码设计的第二个好处
<!-- slide data-notes="" -->
