...
- 指数用"移码"存 (加 127), 负指数也能存成非负数; 位数决定精度: float 约 6-7 位有效数字, double 约 15-16 位
...
```text
验算一个例子: 0.75 = 0.11₂ (1/2 + 1/4) = 1.1 × 2⁻¹

符号 0 | 指数 -1 → 存 127-1 = 126 | 尾数 1000…0 (隐含 1 不存)

存储: 0 01111110 10000000000000000000000

反推: 指数 126-127 = -1; 尾数补回隐含 1 得 1.1₂ = 1.5
      1.5 × 2⁻¹ = 0.75 ✓
```
</div>
...
- 32 位只有 ==2³² ≈ 42.9 亿个图案==, 而数轴无限长——每个图案对应一个点, 只能==稀疏采样==

- 0.1 这类数找不到精确图案, 只能==就近截断==
...
- 每个指数区间里都有==同样多==的数 (float 都是 $2^{23}$ 个): 1~2 有 $2^{23}$ 个, 2~4 也只有 $2^{23}$ 个

- 区间长度 ×2 → 间距 ×2 → 离 0 越远越稀疏; ==约一半的 float 都挤在 -1 到 1 之间==

- 特殊值: ==0 / -0.0== (全 0)、==非规格化数== ($0.xxx \times 2^{-126}$, 填 0 附近的空隙)、==±∞== (指数全 1 尾数 0)、==NaN== (指数全 1 尾数非 0)<!-- slide data-notes="" -->
