...
- 位数决定精度: 23+1=24 位 × 0.301 ≈ 7.2 → float 约 6-7 位有效数字; double 约 15-16 位

- 对比 ==0.1==: 二进制是无限循环小数, 尾数只能截断——存进去的已不是 0.1

下一页: 亲手验算一个 float<!-- slide data-notes="" -->##### 验算一个 float: 0.75

---

1. 二进制: ==0.75 = 0.11₂ = 1/2 + 1/4==
...
3. 三个字段:
   ...
   - 尾数位: ==1000…0== (23 位, 隐含的 1 不存)

4. 存储: ==0 01111110 1000…0 (23 位)==

5. 反推验算: 指数 126−127 = −1; 尾数补回隐含 1 得 1.1₂ = 1.5 → ==1.5 × 2⁻¹ = 0.75 ✓==<!-- slide data-notes="" -->##### 有限图案 → 无限数轴
